...
class ReactTest(unittest.TestCase):
    def test_001_input_cells_have_a_value(self):
        input_ = InputCell(10)
        self.assertEqual(input_.value, 10)

    def test_002_can_set_input_cell_value(self):
        input_ = InputCell(4)
        input_.value = 20
        self.assertEqual(input_.value, 20)

    def test_003_compute_cells_calculate_initial_value(self):
        input_ = InputCell(1)
        output = ComputeCell([input_], lambda inputs: inputs[0] + 1)
        self.assertEqual(output.value, 2)

    def test_004_compute_cells_take_inputs_in_right_order(self):
        one = InputCell(1)
        two = InputCell(2)
        output = ComputeCell([one, two], lambda inputs: inputs[0] + inputs[1] * 10)
        self.assertEqual(output.value, 21)

    def test_005_compute_cells_update_value_when_dependencies_are_changed(self):
        input_ = InputCell(1)
        output = ComputeCell([input_], lambda inputs: inputs[0] + 1)
    ...
    def test_006_compute_cells_can_depend_on_other_compute_cells(self):
        input_ = InputCell(1)
    ...
    def test_007_compute_cells_fire_callbacks(self):
        input_ = InputCell(1)
        output = ComputeCell([input_], lambda inputs: inputs[0] + 1)
    ...
    def test_008_callbacks_only_fire_on_change(self):
        input_ = InputCell(1)
    ...
    def test_009_callbacks_do_not_report_already_reported_values(self):
        input_ = InputCell(1)
        output = ComputeCell([input_], lambda inputs: inputs[0] + 1)
    ...
    def test_010_callbacks_can_fire_from_multiple_cells(self):
        input_ = InputCell(1)
    ...
    def test_011_callbacks_can_be_added_and_removed(self):
        input_ = InputCell(11)
        output = ComputeCell([input_], lambda inputs: inputs[0] + 1)
    ...
    def test_012_removing_a_callback_multiple_times(self):
    ...
    def test_013_callbacks_should_only_be_called_once(self):
    ...
    def test_014_callbacks_not_called_so_long_as_output_not_changed(self):
